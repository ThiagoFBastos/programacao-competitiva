...
impl LCA {
   ...
   fn jump(&self, mut u: usize, levels: i32) -> usize {
      if levels <= 0 {
         return u;
      }

      let lg = 31 - levels.leading_zeros() as usize;
   ...
   fn kth_vertex_path(&self, u: usize, v: usize, mut k: i32) -> Option<usize> {
      let x = self.query(u, v);

      let dist = self.weight[u] + self.weight[v] - 2 * self.weight[x];

      if dist + 1 < k as i64 {
         return None;
      }

      let up = self.weight[u] - self.weight[x] + 1;

      if up >= k as i64 {
         let vertex = self.jump(u, k - 1);
         return Some(vertex);
      }

      k -= up as i32;

      let down = self.weight[v] - self.weight[x] - k as i64;

      let vertex = self.jump(v, down as i32);
...
fn main() {
   ...
   for _ in 0..q {
      ...
      let k = sc.next::<i32>().unwrap();

      let kth = lca.kth_vertex_path(s, t, k + 1);

      if let Some(vertex) = kth {
         writeln!(writer, "{}", vertex).ok();
      } else {
         writeln!(writer, "-1").ok();
      }
   }
