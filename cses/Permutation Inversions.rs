...
fn main() {
   ...
   let mut cnt = vec![ModNumber::<MOD>::new(0); n * (n - 1) / 2 + 2];
   
   cnt[0] = ModNumber::<MOD>::new(1);

   for i in 0..n {
      let mut p = vec![ModNumber::<MOD>::new(0); cnt.len()];

      for j in 0..=k.min((i * i - i) / 2) {
         p[j] += cnt[j];
         p[j + i + 1] -= cnt[j];
      }
      ...
      for j in 1..=i * (i + 1) / 2  {
         if j > k {
            p[j] = ModNumber::<MOD>::new(0);
         } else {
            let (left, right) = p.split_at_mut(j);
            right[0] += left[j - 1];
         }
      }
